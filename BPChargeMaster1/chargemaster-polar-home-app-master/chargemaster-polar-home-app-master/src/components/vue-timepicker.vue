...
export default {
    ...
    methods: {
        ...
        renderList(listType, interval) {
            ...
            if (interval === 0) {
                ...
                window.console.warn(
                    '`' +
                        listType +
                        '-interval` should be less than 60. Current value is',
                    interval
                )
                interval = 1
            } else if (interval < 1) {
                window.console.warn(
                    '`' +
                        listType +
                        '-interval` should be NO less than 1. Current value is',
                    interval
                )
                interval = 1
            } else if (!interval) {
        ...
        fillValues() {
            ...
            const apmValue =
                baseOnTwelveHours && this.apm
                    ? String(this.apm).toLowerCase()
                    : false
                ...
                switch (token) {
                    ...
                    case 'HH':
                        if (!String(hourValue).length) {
                            ...
                            if (apmValue === 'pm') {
                                value =
                                    hourValue < 12 ? hourValue + 12 : hourValue
                            } else {
                                value = hourValue % 12
                            }
                        } else {
