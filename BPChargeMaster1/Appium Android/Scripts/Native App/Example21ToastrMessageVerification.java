...
public class Example21ToastrMessageVerification {
	...
	public static void main(String[] args) throws Exception {
		...
		//below cmd will change the status of that montiored switch
		driver.findElementById("io.appium.android.apis:id/monitored_switch").click();
		
		//as soon as we licekd on that a message is displaying., lets try to capture the screenshot for the same... 
		
		File scrFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
		//store screenshot at a specidfied location
		FileUtils.copyFile(scrFile, new File("C:\\Users\\Srujan\\eclipse-workspace\\Batch98Appium\\images\\depika.png"));
		
		File targetFile=new File("C:\\Users\\Srujan\\eclipse-workspace\\Batch98Appium\\images\\kiran.png");
		
		//below code will read the content from an image which we copied from our webdriver proragmm
		...
		if(s.contains("Monitored switch is on")){
			...
			System.out.println("text is there on the app, means text verified successfully");
		}else{
			System.out.println("desired tesxt is not there please check your captuered screenshot");
		}
